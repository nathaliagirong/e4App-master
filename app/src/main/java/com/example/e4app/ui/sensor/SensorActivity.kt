...
import com.example.e4app.TimerActivity
...
class SensorActivity : Activity(), EmpaDataDelegate, EmpaStatusDelegate{
    ...
    fun createCsvStop() {
        val name = edtPersonName.text.toString()+ "_cancelado" + ".csv"
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (data != null){
            if (resultCode == Activity.RESULT_OK){
                var flagStop: String = data.getStringExtra("flagStop")
                var flagTimer: String = data.getStringExtra("flagTimer")
